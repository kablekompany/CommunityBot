	...
	async execute(interaction) {
		...
		collector.on('collect', async (i) => {
			if (!participants.includes(i.user.id)) {
				...
				userEmos[`<@${i.user.id}>`] =
					emotes[gamemode][
						Math.floor(Math.random() * emotes[gamemode].length)
					];
				await i.reply({
		...
		collector.on('end', async () => {
			await m.edit({
				components: [
					new MessageActionRow({
						components: [join.setDisabled(true)]
					})
				]
			});
			...
			const msg = await interaction.followUp({
				embeds: [
					new Embed({
						footer: {
							text: `There were ${participants.length} participants`
						}
					})
