...
export default new Handler<'guildDelete'>(async function (guild: Guild) {
	const { bootLog } = this.config.log;
	if (!bootLog.enabled) return null;

	const joinChannel = this.bot.channels.resolve(bootLog.channel as Snowflake) as TextChannel;
	const guildOwner = await guild.fetchOwner({ force: true });

	await joinChannel.send({ embeds: [{
