import { Message, MessageEmbed, Snowflake, TextChannel, GuildChannel } from 'discord.js';
import { MessageHandler } from '../../../models/handler/MessageHandler';

export default new MessageHandler(
	async function (msg: Message) {
		const { tradeCategory, dramaWatcher, tradeBuying, tradeSelling } = this.config.dmc;
		...
		const reply = (content: string) => msg.reply({ content })
			.then(m => this.utils.sleep(7500))
			.then(() => msg.delete());
		const logMsg = (reason: string) => (dramas as TextChannel).send({
			embeds: [{
				title: `Reason: ${reason}`,
				author: {
		...
		if (filter(tradeBuying, 'sell')) {
			await msg.delete();
			reply(`this channel is for buying stuff, goto <#${tradeSelling}> to sell.`);
		...
		if (filter(tradeSelling, 'buy')) {
			await msg.delete();
			reply(`this channel is for selling stuff, goto <#${tradeBuying}> to buy.`);
			logMsg('selling in buying-ads');
			return null;
		}
