import { MessageEmbedOptions, MessageOptions, Client, Snowflake } from 'discord.js';
...
export default new Command(
	async ({ ctx, msg, args }): Promise<MessageOptions> => {
		...
		if (ctx.config.owners.includes(msg.author.id)) {
			guild = resolveGuild.call(ctx.bot, (args[1] ? args[0] : guild.id) as Snowflake);
			page = args[1] ?? args[0];
		}

		const emojis = await guild.emojis.fetch().then(emjs => {
			return emjs.map(e => `<${e.animated ? 'a' : ''}:${e.name}:${e.id}> \`:${e.name}:\``)
		});

		const pages = ctx.utils.paginate(emojis);
		if (Number(page) > pages.length) {
			return { content: `Page **${page}** doesn't exist you dingus, there are only ${pages.length} total pages.` };
		}
