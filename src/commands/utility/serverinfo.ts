...
export default class extends Command<Args> {
  ...
  private getGuild(guild: Guild): string {
    ...
    const v = [
      guild.name,
      Formatters.inlineCode(guild.id),
      guild.verificationLevel.toLowerCase(),
    ];
  ...
  private async getOwner(guild: Guild): Promise<string> {
    const { user, nickname, id } = await guild.fetchOwner({
      force: true,
      cache: true,
    });
  ...
  private getStats(guild: Guild): string {
    ...
    const v = [guild.channels, guild.emojis, guild.roles, guild.members].map(
      format,
    );
