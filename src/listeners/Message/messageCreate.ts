...
export default class extends Listener<typeof Events.MessageCreate> {
	...
	private async censors(msg: Message) {
		const { trialMod, modRole, tradeCategory, memerCategory, dramaWatcher } = msg.client.config.dmc;
		const categories = [tradeCategory, memerCategory];
		const modRoles = [trialMod, modRole];

		if (msg.channel.type !== 'GUILD_TEXT')
			return null;
		if (modRoles.some(msg.member!.roles.cache.has))
			return null;
		...
		const dramaChan = await msg.client.channels.fetch(dramaWatcher) as TextChannel;
		return dramaChan.send({
			embeds: [
				{
	...
	private async crossposts(msg: Message) {
		...
		await msg.crosspost();
		return msg.client.logger.info(`Message from sales/lotto channels have been crossposted at ${msg.client.util.prettyDate()}`);
	}
	...
	private async general(msg: Message) {
		...
		const filter = msg.content.toLowerCase().includes('pls') && hasRole(modRole);
		const dramas = await msg.client.channels.fetch(dramaWatcher) as TextChannel;
	...
	private async trades(msg: Message) {
		...
		if (msg.channel.type !== 'GUILD_TEXT')
			return null;
		if (modRoles.some((mr) => msg.member!.roles.cache.has(mr)))
			return null;
		if (!(msg.channel as GuildChannel).parentId)
			return null;
		if (!categories.includes((msg.channel as GuildChannel).parentId!))
			return null;
