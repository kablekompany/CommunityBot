...
export class ModCommand extends Command {
   ...
   */
  public constructor(fn: CommandFunction, props: ModCommandProps) {
    super(fn, props);
  }
  ...
  public static checkHierarchy(msg: Message, user: User | GuildMember) {
    const target =
      user instanceof User ? msg.guild.members.resolve(msg.author.id) : user;
    const getHighestPos = (member: GuildMember) =>
