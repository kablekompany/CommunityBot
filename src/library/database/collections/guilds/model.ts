...
export default (collection: GuildCollection): GuildModel => {
	...
	GuildSchema.statics.getGuild = async function (this: GuildModel, id: string) {
		const thisGuild = await this.findById({ _id: id }) ?? await this.create({ _id: id });
		return thisGuild;
	}
	 ...
	 */
	GuildSchema.methods.setPrefix = function (this: GuildDocument, prefix: string) {
		this.prefix = prefix;
		return this;
	}
	 ...
	 */
	GuildSchema.methods.incCommands = function (this: GuildDocument, by?: number) {
		this.commands += by ?? 1;
		return this;
	}
