...
export class BaseCollection<D extends IDocument, M extends IModel<D>> extends Collection<string, D> {
	public db: Database;
	public model: Model<D>;
	public constructor(db: Database, model: (collection: BaseCollection<D, M>) => Model<D>) {
		super();
		this.db = db;
	...
	public async fetchAll(cache = false): Promise<D[]> {
		...
		if (cache) for (const doc of docs) {
